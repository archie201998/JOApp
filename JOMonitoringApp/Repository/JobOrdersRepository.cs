...
namespace JOMonitoringApp
{
    internal class JobOrdersRepository : IJobOrdersRepository
    {
        ...
        public DataTable GetViewRecordsByParameters(string searchText, bool showInactiveTaxpayers, int rowFilter)
        {
            ...
            string query = $"SELECT id, date, account_number, customers_id, account_name, address, particulars_id, particular, or_number, amount, prepared_by_id, prepared_by FROM {viewTableName} WHERE account_number LIKE @search_text OR account_name LIKE @search_text OR date LIKE @search_text ORDER BY date ASC LIMIT @row_filter";
